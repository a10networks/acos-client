...
class Action(base.BaseV30):
    ...
    def setInterface(self, interface, address_list, ip_version):
        ...
        if address_list and ip_version==4:
            data = {'ethernet': {'ifnum': str(interface), "name": "DataPort", "action": "enable", 'ip': {'address-list': address_list}}}
        
        if address_list and ip_version==6:
            data = {'ethernet': {'ifnum': str(interface), "name": "DataPort", 'ipv6': {'address-list': address_list, 'ipv6-enable': 1}}}
