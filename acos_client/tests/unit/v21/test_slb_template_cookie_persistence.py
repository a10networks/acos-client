...
SLB_CREATE_URL = '{}slb.template.cookie_persistence.create&session_id={}'.format(BASE_URL, 'foobar')
SLB_DELETE_URL = '{}slb.template.cookie_persistence.delete&session_id={}'.format(BASE_URL, 'foobar')
SLB_SEARCH_URL = '{}slb.template.cookie_persistence.search&session_id={}'.format(BASE_URL, 'foobar')
...
class TestSLBTemplateCookiePersistence(unittest.TestCase):
    ...
    def test_slb_template_persistence_create(self):
        ...
        responses.add(responses.POST, SLB_CREATE_URL, json=json_response, status=200)
        ...
        self.assertEqual(responses.calls[1].request.url, SLB_CREATE_URL)
    ...
    def test_slb_template_persistence_delete(self):
        ...
        responses.add(responses.POST, SLB_DELETE_URL, json=json_response, status=200)
        ...
        self.assertEqual(len(responses.calls), 2)
        self.assertEqual(responses.calls[1].request.method, responses.POST)
        self.assertEqual(responses.calls[1].request.url, SLB_DELETE_URL)
    ...
    def test_slb_template_persistence_search(self):
        ...
        responses.add(responses.POST, SLB_SEARCH_URL, json=json_response, status=200)
        ...
        self.assertEqual(len(responses.calls), 2)
        self.assertEqual(responses.calls[1].request.method, responses.POST)
        self.assertEqual(responses.calls[1].request.url, SLB_SEARCH_URL)

    @responses.activate
    def test_slb_template_persistence_search_not_found(self):
        responses.add(responses.POST, SLB_SEARCH_URL, json={'session_id': 'foobar'})
        json_response = {
            'response': {'status': 'fail', 'err': {'code': 67371009, 'msg': ' No such Template'}}
        }
        responses.add(responses.POST, SLB_SEARCH_URL, json=json_response, status=200)
